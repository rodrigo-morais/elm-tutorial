...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var httpTask = $Http.getString("http://localhost:3000/");
   var OnRefresh = function (a) {    return {ctor: "OnRefresh",_0: a};};
   var refreshFx = $Effects.task(A2($Task.map,OnRefresh,$Task.toResult(httpTask)));
   var init = {ctor: "_Tuple2",_0: "",_1: refreshFx};
   var update = F2(function (action,model) {
      var _p0 = A2($Debug.log,"action",action);
      switch (_p0.ctor)
      {case "Refresh": return {ctor: "_Tuple2",_0: model,_1: refreshFx};
         case "OnRefresh": var message = A2($Result.withDefault,"",_p0._0);
           return {ctor: "_Tuple2",_0: message,_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var Refresh = {ctor: "Refresh"};
   var view = F2(function (address,message) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Refresh)]),_U.list([$Html.text("Refresh")])),$Html.text(message)]));
   });
