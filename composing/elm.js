...
Elm.Widget.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      if (_p0.ctor === "Increase") {
            return _U.update(model,{count: model.count + 1});
         } else {
            return _U.cmp(model.count,0) > 0 ? _U.update(model,{count: model.count - 1}) : model;
         }
   });
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model.count))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increase)]),_U.list([$Html.text("Increase")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrease)]),_U.list([$Html.text("Decrease")]))]));
   });
   var initialModel = {count: 0};
   var Model = function (a) {    return {count: a};};
   return _elm.Widget.values = {_op: _op,Model: Model,initialModel: initialModel,Increase: Increase,Decrease: Decrease,view: view,update: update};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      var updatedWidgetModel = A2($Widget.update,_p0._0,model.widgetModel);
      return _U.update(model,{widgetModel: updatedWidgetModel});
   });
   var WidgetAction = function (a) {    return {ctor: "WidgetAction",_0: a};};
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([A2($Widget.view,A2($Signal.forwardTo,address,WidgetAction),model.widgetModel)]));
   });
   var initialModel = {widgetModel: $Widget.initialModel};
   var main = $StartApp$Simple.start({model: initialModel,view: view,update: update});
   var AppModel = function (a) {    return {widgetModel: a};};
   return _elm.Main.values = {_op: _op,AppModel: AppModel,initialModel: initialModel,WidgetAction: WidgetAction,view: view,update: update,main: main};
};
