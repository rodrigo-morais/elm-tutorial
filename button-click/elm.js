...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "NoOp") {
            return A2($Debug.log,"NoOp",model);
         } else {
            return A2($Debug.log,"Increase",_U.update(model,{count: model.count + 1}));
         }
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model.count))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increase)]),_U.list([$Html.text("Click")]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var mb = $Signal.mailbox(NoOp);
   var modelSignal = A3($Signal.foldp,update,initialModel,mb.signal);
   var main = A2($Signal.map,view(mb.address),modelSignal);
